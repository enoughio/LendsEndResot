...
const StayDetailsCard = ({ imageHidden = false }: { imageHidden?: boolean }) => {
  return (
    <div className="-translate-y-[20%] md:-translate-y-[35%] w-[85%] sm:w-[45%] md:w-[90%] lg:w-[80%] mx-auto bg-white h-96 md:h-40 px-6 py-6 lg:py-7  shadow-lg rounded-2xl sm:rounded-xl flex flex-col items-center md:items-start justify-between text-[#112211]">
      <div
        className={`hidden sm:${
      ...
      <div className="w-full h-[80%] md:h-fit flex flex-col md:flex-row items-center justify-between">
        ...
        <div className="w-[90%] sm:w-[60%] md:w-fit flex items-center gap-2 md:gap-5 text-[12px] lg:text-sm">
          <p className="hidden md:inline-flex hover:cursor-pointer">
            <Image src={"/add icon.svg"} alt="" width={16} height={16} />
            Add Promo Code
          </p>
          <button className="w-full md:w-fit bg-[#067C0B] text-white rounded-sm p-3 inline-flex items-center justify-center gap-1 lg:mr-3 hover:bg-green-800 active:bg-green-800 hover:cursor-pointer hover:scale-105 active:scale-100 transition-transform duration-200">
